...
parser.add_argument('--model', type=str, default='mutil_attention_model', help='model_name')
parser.add_argument('--max_iteration', type=int, default=15000, help='maximum iteration to train')
parser.add_argument('--max_samples', type=int, default=140,help='maximum samples to train')
...
parser.add_argument('--labelnum', type=int, default=14, help='trained samples')
parser.add_argument('--seed', type=int, default=1337, help='random seed')
parser.add_argument('--gpu', type=str, default='0', help='GPU to use')
parser.add_argument('--lamda', type=float, default=0.333, help='weight to balance supervised loss')
...
if __name__ == "__main__":
    ...
    # lovasz_loss = losses.LovaszLossSoftmax()
    iter_num = 0
    best_dice = 0
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            model.train()
            outputs_x, outputs_y, outputs_z, embedding_x, embedding_y, embedding_z = model(volume_batch)
            outputs_list = [outputs_x, outputs_y, outputs_z]
            ...
            # outputs_x_soft = F.softmax(outputs_x, dim=1)  # [batch, num_class, h, w, d]
            # outputs_y_soft = F.softmax(outputs_y, dim=1)  # soft prediction of fa
            # outputs_z_soft = F.softmax(outputs_z, dim=1)  # soft prediction of fa
            #
            # labeled_features_x = embedding_x[:args.labeled_bs, ...]
            # labeled_features_y = embedding_y[:args.labeled_bs, ...]
            # labeled_features_z = embedding_z[:args.labeled_bs, ...]
            #
            # # unlabeled embeddings to calculate correlation matrix with embeddings sampled from the memory bank
            # unlabeled_features_x = embedding_x[args.labeled_bs:, ...]
            # unlabeled_features_y = embedding_y[args.labeled_bs:, ...]
            # unlabeled_features_z = embedding_z[args.labeled_bs:, ...]
            #
            # y_x = outputs_x_soft[:args.labeled_bs]
            # y_y = outputs_y_soft[:args.labeled_bs]
            # y_z = outputs_z_soft[:args.labeled_bs]
            #
            # true_labels = label_batch[:args.labeled_bs]
            #
            # _, prediction_label_x = torch.max(y_x, dim=1)
            # _, prediction_label_y = torch.max(y_y, dim=1)
            # _, prediction_label_z = torch.max(y_z, dim=1)
            #
            # predicted_unlabel_prob_x, predicted_unlabel_x = torch.max(outputs_x_soft[args.labeled_bs:],
            #                                                           dim=1)  # v_unlabeled_mask
            # predicted_unlabel_prob_y, predicted_unlabel_y = torch.max(outputs_y_soft[args.labeled_bs:],
            #                                                           dim=1)  # a_unlabeled_mask
            # predicted_unlabel_prob_z, predicted_unlabel_z = torch.max(outputs_y_soft[args.labeled_bs:],
            #                                                           dim=1)  # a_unlabeled_mask
            #
            #
            # # Select the correct predictions including the foreground class and the background class
            # mask_prediction_correctly = (
            #         ((prediction_label_x == true_labels).float() + (prediction_label_y == true_labels).float() + (prediction_label_y == true_labels).float()) == 3)
            #
            # labeled_features_x = labeled_features_x.permute(0, 2, 3, 4, 1).contiguous()
            # b, h, w, d, labeled_features_dim = labeled_features_x.shape
            #
            # # get projected features
            # model.eval()
            # proj_labeled_features_x = model.projection_head1(labeled_features_x.view(-1, labeled_features_dim))
            # proj_labeled_features_x = proj_labeled_features_x.view(b, h, w, d, -1)
            #
            # proj_labeled_features_y = model.projection_head2(labeled_features_y.view(-1, labeled_features_dim))
            # proj_labeled_features_y = proj_labeled_features_y.view(b, h, w, d, -1)
            #
            # proj_labeled_features_z = model.projection_head2(labeled_features_z.view(-1, labeled_features_dim))
            # proj_labeled_features_z = proj_labeled_features_z.view(b, h, w, d, -1)
            #
            # model.train()
            #
            # labels_correct_list = []
            ...
            #     labeled_features_correct_list.append((proj_labeled_features_x[i][mask_prediction_correctly[i]] +
            #                                           proj_labeled_features_y[i][mask_prediction_correctly[i]] +
            #                                           proj_labeled_features_z[i][mask_prediction_correctly[i]]) / 3)
            ...
            # mask_consist_unlabeled = (predicted_unlabel_x == predicted_unlabel_y).to(torch.int64) == predicted_unlabel_z  # [b, h, w, d]
            # # use model V's predicted label and prob to filter unlabeled feature online
            # consist_unlabel = predicted_unlabel_x[mask_consist_unlabeled]  # [num_consist]
            # consist_unlabel_prob = predicted_unlabel_prob_x[mask_consist_unlabeled]  # [num_consist]
            #
            # unlabeled_features_x = unlabeled_features_x.permute(0, 2, 3, 4, 1)
            # unlabeled_features_y = unlabeled_features_y.permute(0, 2, 3, 4, 1)
            # unlabeled_features_z = unlabeled_features_z.permute(0, 2, 3, 4, 1)
            #
            #
            # unlabeled_features_x = unlabeled_features_x[mask_consist_unlabeled, :]  # [num_consist, feat_dim]
            # unlabeled_features_y = unlabeled_features_y[mask_consist_unlabeled, :]
            # unlabeled_features_z = unlabeled_features_z[mask_consist_unlabeled, :]
            #
            # # get fv's correlation matrix
            # projected_feature_x = model.projection_head1(unlabeled_features_x)
            # predicted_feature_x = model.prediction_head1(projected_feature_x)
            # corr_x, corr_x_available = correlation.cal_correlation_matrix(predicted_feature_x,
            ...
            # projected_feature_y = model.projection_head2(unlabeled_features_y)
            # predicted_feature_y = model.prediction_head2(projected_feature_y)
            # corr_y, corr_y_available = correlation.cal_correlation_matrix(predicted_feature_y,
            #                                                               consist_unlabel_prob,
            #                                                               consist_unlabel,
            #                                                               memory,
            #                                                               num_classes,
            #                                                               num_filtered=args.num_filtered)
            #
            # projected_feature_z = model.projection_head3(unlabeled_features_z)
            # predicted_feature_z = model.prediction_head3(projected_feature_z)
            # corr_z, corr_z_available = correlation.cal_correlation_matrix(predicted_feature_z,
            #                                                               consist_unlabel_prob,
            #                                                               consist_unlabel,
            #                                                               memory,
            #                                                               num_classes,
            ...
            # if corr_x_available and corr_y_available and corr_z_available:
            #     num_samples = corr_x.shape[0]
            #     loss_o = torch.sum(torch.sum(-corr_x * torch.log(corr_y + 1e-8), dim=1)) / num_samples
            # else:
            #     loss_o = 0
            loss_o = 0
            iter_num = iter_num + 1
