...
class LAHeart(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None, with_idx=False):
        ...
        train_path = self._base_dir + r'\train.list'
        test_path = self._base_dir + r'\test.list'
...
class LeafSpot(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None, with_idx=False):
        ...
        if split == 'train':
            train_path = self._base_dir + '/training/image'
            self.image_list = os.listdir(train_path)
        elif split == 'test':
            test_path = self._base_dir + '/test/image'
            self.image_list = os.listdir(test_path)
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(self._base_dir + '/training/image/'+ image_name, -1)
        label = cv2.imread(self._base_dir + '/training/label/' + image_name.split('.')[0] + '_scab.png', -1)[...,np.newaxis]
        ...
        if self.transform:
            ...
            # end = np.random.randint(256,512)
            # image = sample['image'].transpose(2, 0, 1)[:,end-256:end,end-256:end]
            # label = sample['label'].transpose(2, 0, 1)[:,end-256:end,end-256:end]
...
class Kvasir_SEG(Dataset):
    ...
    def __init__(self,_base_dir=None, train_list=None, split='train', num=None, transform=None, with_idx=False):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(self._base_dir + '/images/' + image_name)
        image = np.array(image)
        label = Image.open(self._base_dir + '/masks/' + image_name)
...
class Pancreas(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None, with_idx=None):
        self._base_dir = base_dir
        self.transform = transform
        self.sample_list = []
        self.with_idx = with_idx
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        image = image.reshape(image.shape[0], image.shape[1], image.shape[2]).astype(np.float32)
