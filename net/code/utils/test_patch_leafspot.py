...
def var_all_case(model, num_classes, patch_size=(32, 32), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
    ...
    elif dataset_name == "zaoyibing":
        test_path = '/home/felicia/datasets/leaf_spots/' + dataset_name + '/validation/image'
        image_list = os.listdir(test_path)
    elif dataset_name == "Kvasir_SEG":
        test_path = '/home/felicia/datasets/Kvasir_SEG/images'
        image_list = os.listdir(test_path)

    image_list = [item.replace('\n', '') for item in image_list]

    if dataset_name == "Kvasir_SEG":
        image_list = image_list[700:900]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        image = cv2.imread('/home/felicia/datasets/Kvasir_SEG/images/' + image_path, -1)
        label = cv2.imread('/home/felicia/datasets/Kvasir_SEG/masks/' + image_path, -1)[...,0,np.newaxis]
        ...
        label = label[end1:end1+512,end2:end2+512,:]

        label = np.where(label >= 127, 1, 0).transpose(2,0,1)
        # h5f = h5py.File(image_path, 'r')
        # image = h5f['image'][:]
        # label = h5f['label'][:]
        prediction = test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        dice = metric.binary.dc(prediction[0,1,...], label)
        total_dice += dice
...
def test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    test_patch = (image / 255).transpose(2, 0, 1)[np.newaxis, ...]
    cnt = np.zeros(image.shape).astype(np.float32)
    test_patch = torch.from_numpy(test_patch).cuda().to(torch.float)
    score_map= 0
    with torch.no_grad():
        y = model(test_patch)
        if len(y) > 1:
