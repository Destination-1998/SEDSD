...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open(r'D:\lixin\data\LA\test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["D:\\lixin\\data\LA\\data\\" + item.replace('\n', '') + r"\mri_norm2.h5" for item in
                      image_list]
        ...
        with open('/home/felicia/datasets/ACDC/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["/home/felicia/datasets/ACDC/data/" + item.replace('\n', '') + ".h5" for item in image_list]
    elif dataset_name == "Pancreas":
        with open(r'D:\lixin\data\Pancreas\test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["D:\\lixin\\data\Pancreas\\data\\" + item.replace('\n', '') + r"_norm.h5" for item in
                      image_list]
    ...
    for image_path in loader:
        ...
        prediction1, prediction2, score_map1, score_map2 = test_single_case_first_output(model, image, stride_xy,
                                                                                         stride_z, patch_size,
        ...
        if np.sum(prediction1) == 0:
            ...
            metric1 = calculate_metric_percase(prediction1, label)

        if np.sum(prediction2) == 0:
            metric2 = 0
        else:
            metric2 = calculate_metric_percase(prediction2, label)
    ...
    print('average metric of dice1 is {}'.format(avg_dice1))
    print('average metric of dice2 is {}'.format(avg_dice2))
...
def test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    cnt1 = np.zeros(image.shape).astype(np.float32)
    cnt2 = np.zeros(image.shape).astype(np.float32)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                test_patch = torch.cat(
                    [torch.from_numpy(test_patch), torch.zeros_like(torch.from_numpy(test_patch))]).cuda()

                with torch.no_grad():
                    f1, f2, y1, y2 = model(test_patch)
                    # y1, y2 = model(test_patch)
                    # if len(y2) > 1:
                    #     #y = y[0]
                    #     y = y[-1]
                ...
                cnt1[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                    = cnt1[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + 1
                cnt2[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                    = cnt2[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + 1

    score_map1 = score_map1 / np.expand_dims(cnt1, axis=0)
    score_map2 = score_map2 / np.expand_dims(cnt2, axis=0)
    ...
    return label_map1, label_map2, score_map1, score_map2
